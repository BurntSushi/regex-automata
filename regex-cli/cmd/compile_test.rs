...
The directory given to this command is where a bunch of different Cargo
projects will be written.

USAGE:
    regex-cli compile-test <outdir>
...
"#;
    ...
    writeln!(wtr, "name,crate,version,profile,duration,size,relative-size")?;
    ...
    fn configure(
        ...
        match *arg {
            ...
            Arg::Value(ref mut v) => {
                anyhow::ensure!(
                    self.outdir.is_none(),
                    "<outdir> was given multiple times",
                );
...
When this is not given, only 'regex' tests will be run.
"#,
        )];
        USAGES
    }
...
struct Test {
    ...
    features: Vec<String>,
    /// Whether we're testing the regex-automata crate or not. When not, then
    /// we're testing the regex crate.
    regex_automata: bool,
    /// The version of the regex (or regex-automata) crate to test.
    ///
    /// Note that this test runner doesn't really know about versions. So the
    /// crate features it tests might not work if they weren't introduced in
...
impl Test {
    ...
    fn name(&self) -> String {
        let krate =
            if self.regex_automata { "regex-automata" } else { "regex" };
...
[patch.crates-io]
aho-corasick = {{ path = "/home/andrew/rust/aho-corasick" }}
