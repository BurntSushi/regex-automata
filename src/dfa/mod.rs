...
* A [`dense::DFA`](dense/struct.DFA.html) provides low level access to a DFA
  that uses a dense representation (uses lots of space, but fast searching).
* A [`sparse::DFA`](sparse/struct.DFA.html) provides the same API as a
  `dense::DFA`, but uses a sparse representation (uses less space, but slower
  matching).
* A [`Automaton`](trait.Automaton.html) trait that defines an interface that
  all DFAs must implement.
* Both dense DFAs and sparse DFAs support
  [serialization to raw bytes](dense/struct.DFA.html#method.to_bytes_little_endian)
  and
  [cheap deserialization](dense/struct.DFA.html#method.from_bytes).
...
* We specifically convert the dense DFA to a representation that uses `u16`
  ...
  [`dense::DFA::to_sized`](dense/struct.DFA.html#method.to_sized). While this
  isn't strictly necessary, if we skipped this step, then the serialized bytes
  would use `usize` for state identifiers, which does not have a fixed size.
  Using `u16` ensures that we can deserialize this DFA even on platforms with a
...
* To convert the DFA to raw bytes, we use the `to_bytes_native_endian`
  ...
  [`dense::DFA::to_bytes_little_endian`](dense/struct.DFA.html#method.to_bytes_little_endian)
  or
  [`dense::DFA::to_bytes_big_endian`](dense/struct.DFA.html#method.to_bytes_big_endian),
...
// now deserialize both---we need to specify the correct type!
let fwd: sparse::DFA<&[u8], u16> = unsafe {
    sparse::DFA::from_bytes_unchecked(&fwd_bytes)?.0
};
let rev: sparse::DFA<&[u8], u16> = unsafe {
    sparse::DFA::from_bytes_unchecked(&rev_bytes)?.0
};
// finally, reconstruct our regex
let re2 = Regex::from_dfas(fwd, rev);
...
* This crate exposes DFAs directly, such as
  ...
  and
  [`sparse::DFA`](dense/struct.DFA.html),
...
///
/// This module is the home of [`dense::DFA`](struct.DFA.html) and each of its
...
///
/// This module is the home of [`sparse::DFA`](struct.DFA.html).
///
...
/// [constructor](struct.DFA.html#method.new),
/// or by first
/// [configuring the construction of a dense DFA](../dense/struct.Builder.html)
/// and then calling
/// [`dense::DFA::to_sparse`](../dense/struct.DFA.html#method.to_sparse). For
