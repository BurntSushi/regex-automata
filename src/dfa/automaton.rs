...
pub unsafe trait Automaton {
    ...
    /// accelerated state.
    ///
    /// An accelerated state is a special optimization trick implemented by
    /// this crate. Namely, if
    /// [`dense::Config::acelerate`](crate::dfa::dense::Config::accelerate)
    /// is enabled (and it is by default), then DFAs generated by this crate
    /// will tag states meeting certain characteristics as accelerated. States
    /// meet this criteria whenever most of their transitions are self-transitions.
    ...
    /// that state. Thus, there is an opportunity to look for those bytes using
    /// more optimized routines rather than continuing to run through the DFA.
    /// This trick is similar to the prefilter idea described in the documentation
    /// of [`Automaton::is_start_state`], except that while it is more limited
    /// (it only applies to single bytes), it can occur anywhere in the DFA.
    /// This increases the optimization opportunities.
