...
use crate::{
    ...
    util::{
        ...
        prefilter,
        search::{HalfMatch, MatchError, MatchKind, Search},
...
impl DFA {
    ...
    /// let got = dfa.try_find_overlapping_fwd(
    ///     &mut cache, haystack, &mut state,
    /// )?;
    /// assert_eq!(expected, got);
    ///
    ...
    /// ```
    #[inline]
    pub fn try_find_overlapping_fwd<H: AsRef<[u8]>>(
        &self,
        cache: &mut Cache,
        haystack: H,
...
impl DFA {
    ...
    /// match occurred.
    ///
    /// This is like [`DFA::try_find_overlapping_fwd`], except it provides
    /// some additional control over how the search is executed.
    ///
...
impl DFA {
    ...
    pub fn start_state_forward(
        ...
        let sid = LazyRef::new(self, cache)
            .get_cached_start_id(search.get_pattern(), start_type);
        if !sid.is_unknown() {
            return Ok(sid);
        }
        Lazy::new(self, cache)
            .cache_start_group(search.get_pattern(), start_type)
    }
